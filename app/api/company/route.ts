...
export async function POST(request:NextRequest) {
    try {
        ...
        const {name,email,description,website,logo,location,industry} : ICompany = await request.json();
        
        if (!name || !email || !description || !website || !logo || !location || !industry) {
        ...
        if (companyExists) {
            return NextResponse.json({ message: "Company already exists", success: false }, { status: 400 });
        } else {
             await Company.create({name,email,description,website,logo,location,industry});
            return NextResponse.json({ message: "Company created successfully", success: true }, { status: 200 });
